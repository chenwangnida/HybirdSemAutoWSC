...
public class LocalOntologyContainer {
	...
	private  void debugDisplay(String string) {
		if (debug)
			hybirdsem.owlssm.io.ErrorLog.debug(string);
	}
...
//    	return local.createOntProperty(prop.getURI());
    ...
    private OntProperty createOnProperty(Restriction r){
    	try{
    		...
    		//e.printStackTrace();
    		hybirdsem.owlssm.io.ErrorLog.instanceOf().report(this.getClass().toString() + "|createOnProperty: Couldn't create onProperty for restriction " + r.toString());
    		hybirdsem.owlssm.io.ErrorLog.instanceOf().report("                    returned null instead" );
    		return null;
    	}
    }
...
//			subClass.setSuperClass(superClass);
    ...
    boolean processClass(OWLKnowledgeBase kbase, URI clazzURI) {
    	...
    	if (clazz==null) {
    		hybirdsem.owlssm.io.ErrorLog.instanceOf().report("Couldn't find the clazz " + clazzURI + " in base " + kbase);
    		return false;
    	}
