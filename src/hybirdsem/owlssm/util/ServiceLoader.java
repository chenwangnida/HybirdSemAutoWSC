...
//			 Service service = kb.readService(uri);
			...
			if (service == null) {
				System.out.println("Loading service" + "Could not load service " + uri.toString() + "\n"
			...
			System.err.println("Couldn't read service " + uri.toString()
	...
	private void saveServiceItemInfo(ServiceContent item) {
		try {
			...
			String line = uri + " (" + item.inputs.size() + "/" + item.outputs.size()
					+ ") (#InputConcepts/#OutputConcepts)";
